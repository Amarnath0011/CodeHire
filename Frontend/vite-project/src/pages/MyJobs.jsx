    ...
    <section className="min-h-screen bg-slate-950 text-white px-4 sm:px-6 md:px-10 py-10 relative overflow-hidden">
      ...
      <div className="relative z-10 max-w-7xl mx-auto">
        <div className="bg-gradient-to-r from-blue-600 to-indigo-600 rounded-3xl p-6 sm:p-8 shadow-2xl shadow-blue-500/20">
          ...
          <p className="text-white/90 mt-4 max-w-2xl">Manage all your active job openings, review applicants and remove outdated listings.</p>
        </div>
        ...
        {!loading && jobs.length > 0 && (
          <div className="grid md:grid-cols-2 xl:grid-cols-3 gap-6 mt-8">
            {jobs.map((job) => (
              <div key={job._id} className="bg-white/5 border border-white/10 rounded-3xl p-6 backdrop-blur-xl hover:-translate-y-1 hover:bg-white/10 transition duration-300">
                ...
                <div className="grid grid-cols-2 gap-3 mt-6">
                  <Link to={`/applicants/${job._id}`} className="bg-blue-600 hover:bg-blue-700 py-3 rounded-xl font-semibold text-center transition">Applicants</Link>
                  <button onClick={() => handleDelete(job._id)} className="bg-red-600 hover:bg-red-700 py-3 rounded-xl font-semibold transition">Delete</button>
                </div>
              </div>
            ))}
          </div>
        ...
        {!loading && jobs.length === 0 && (
          <div className="text-center mt-16">
            ...
            <Link to="/post-job" className="inline-block mt-6 bg-blue-600 hover:bg-blue-700 px-6 py-3 rounded-xl font-semibold transition">Post New Job</Link>
          </div>
        )}
      </div>
    </section>
